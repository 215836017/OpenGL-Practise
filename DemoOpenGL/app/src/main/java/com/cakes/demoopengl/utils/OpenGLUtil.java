...
public class OpenGLUtil {
    ...
    public static int compileShaderCode(int type, String shaderCode) {
        LogUtil.d(TAG, "compileShaderCode() -- start, shaderCode = " + shaderCode);
        //得到一个着色器的ID。主要是对ID进行操作
        int shaderObjectId = GLES20.glCreateShader(type);
        LogUtil.i(TAG, "compileShaderCode() -- shaderObjectId = " + shaderObjectId);
        ...
        if (shaderObjectId != 0) {
            ...
            LogUtil.d(TAG, "compileShaderCode() -- 22222222222222");
            //2.查询编译的状态
            ...
            if (status[0] == 0) { //等于0。则表示失败
                ...
                LogUtil.w(TAG, "compileShaderCode() -- compile failed!");
                return 0;
            } else {
                LogUtil.i(TAG, "compileShaderCode() -- 3333333");
            }
        }
